...
const ProfilePage = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      await backendApi.post('/addresses/', formData);
      fetchAddresses();
      setFormData({
        address_line_1: '',
        address_line_2: '',
  ...
  const setDefaultAddress = async (addressId) => {
    try {
      await backendApi.put(`/addresses/${addressId}`, { is_default: true });
      fetchAddresses();
    } catch (err) {
  ...
  return (
    <>
      ...
      <div className="profile-page">
              ...
              {addresses.map((address) => (
                <li key={address.id} className={`address-item ${address.is_default ? 'default' : ''}`}>
                  ...
                  <div className="address-actions">
                    {!address.is_default && (
                      <button onClick={() => setDefaultAddress(address.id)} className="btn btn-sm">
                        Set as Default
                      </button>
                    )}
                    <button onClick={() => deleteAddress(address.id)} className="btn btn-sm btn-danger">
                      Delete
                    </button>
                  </div>
                </li>
        ...
        <Link to={getDashboardPath(user?.role)} className="btn btn-link mt-4">
          Back to Dashboard
        </Link>
      </div>
    </>
